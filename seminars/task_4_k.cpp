...
template<class E>
std::vector<sgtl::vertex> getMaxIndependent(sgtl::Network<E>& g, int n) {
    sgtl::Dinitz<sgtl::Network<E>>::getMaxFlow(g);
    ...
    /// thats all
    mGuests.insert(mGuests.end(), nGuests.begin(), nGuests.end());
    std::sort(mGuests.begin(), mGuests.end());
    std::vector<sgtl::vertex> ans;
    for (size_t i = 0; i + 1 < mGuests.size(); ++i) {
        if (mGuests[i] == mGuests[i + 1]) {
            ans.push_back(mGuests[i]);
        }
    }
...
int main() {
    ...
    vector <int> v1(n);
    vector <int> v2(m);
    Network<EdgeCostFlowMarked> g(n + m + 2, n + m, n + m + 1);
    for (int i = 0; i < n; ++i) {
        cin >> v1[i];
        g.pushEdge(EdgeCostFlowMarked(g.getSource(), i, 1, 0, -1));
    }
    for (int i = 0; i < m; ++i) {
        cin >> v2[i];
        g.pushEdge(EdgeCostFlowMarked(n + i, g.getTarget(), 1, 0, -1));
    }
    vector<EdgeCostFlowMarked> pp;
    for (int i = 0; i < e; ++i) {
        int fr, to;
        cin >> fr >> to;
        --fr;
        --to;
        pp.push_back(EdgeCostFlowMarked(fr, n + to, 1, 0 - (v1[fr] + v2[to]), i + 1));
    }
    ...
    for (int i = 0; i < n; ++i) {
        for (auto e = g.begin(i); e != g.end(i); ++e) {
            if ((n <= (*e).to()) && ((*e).to() < n + m) && (!(*e).unsaturated())) {
                ans.push_back((*e).mark);
            }
        }
    }
    cout << 0 - cost << "\n" << ans.size() << "\n";
