...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        if (demage[i] > 0) {
            g.pushEdge(EdgeFlow(i, g.getTarget(), demage[i]));
        } else {
            ans += demage[i];
            g.pushEdge(EdgeFlow(g.getSource(), i, 0. - demage[i]));
        }
    }

    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < depend[i].size(); ++j) {
            g.pushEdge(EdgeFlow(i ,depend[i][j], INF));
        }
    }



    ans += Dinitz::getMaxFlow(g);
    cout << 0 - ans << "\n";
/*
