...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            if ((i + j) % 2) {
                int valence = getValense(s[i][j]);
                maxFlowS += valence;
                g.pushEdge(sgtl::EdgeFlow(g.getSource(), getVertex(i, j), valence));

                for (int iShift = -1; iShift <= 1; ++iShift)
                for (int jShift = -1; jShift <= 1; ++jShift)
                if (abs(iShift) + abs(jShift) == 1) {
                    int x = i + iShift;
                    int y = j + jShift;
                    if (inBound(x, y)) {
                        g.pushEdge(sgtl::EdgeFlow(getVertex(i, j), getVertex(x, y), 1));
                    }
                }
                ...
                int valence = getValense(s[i][j]);
                maxFlowT += valence;
                g.pushEdge(sgtl::EdgeFlow(getVertex(i, j), g.getTarget(), valence));
            }
        }
    }
    if ((sgtl::Dinitz::getMaxFlow(g) == maxFlowS)
            && (maxFlowS == maxFlowT) && (maxFlowS != 0)) {
        std::cout << "Valid";
    } else {
        std::cout << "Invalid";
    }
