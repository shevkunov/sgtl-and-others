...
inline bool chkUndef(char c) {
    return c == '?';
}

inline bool chkZero(char c) {
    return c == '0';
}
...
int main() {
    ...
    size_t strSumLen = s.length() + p.length();
    sgtl::Network g(strSumLen + 2, strSumLen, strSumLen + 1);

    size_t stableHamm = 0;
    for (size_t shift = 0; shift + p.length() <= s.length(); ++shift) {
        for (size_t i = 0; i < p.length(); ++i) {
            if (chkUndef(p[i])) {
                if (chkUndef(s[shift + i])) {
                    g.pushEdge(sgtl::EdgeFlow(i, p.size()+  shift + i, 1));
                    g.pushEdge(sgtl::EdgeFlow(p.size() + shift + i, i, 1));
                } else {
                    if (chkZero(s[shift + i])) {
                ...
                if (chkUndef(s[shift + i])) {
                    if (chkZero(p[i])) {
                        g.pushEdge(sgtl::EdgeFlow(g.getSource(), p.size() + shift + i, 1));
                    } else {
                        // p[i] = '1'
                        g.pushEdge(sgtl::EdgeFlow(p.size() + shift + i, g.getTarget(), 1));
                    }
                } else {
                    stableHamm += (s[i + shift] != p[i]);
                }
            }
        }
    }

    stableHamm += sgtl::Dinitz::getMaxFlow(g);

    std::vector<sgtl::vertex> ordered;
    sgtl::BFS::bfsUntil(g, g.getSource(), g.getTarget(), ordered);
