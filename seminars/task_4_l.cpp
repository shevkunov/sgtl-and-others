...
int main() {
    ...
    Network<EdgeFlow> g(2*n + 2, 2*n, 2*n + 1);
    for (int i = 0; i < n; ++i) {
        g.pushEdge(EdgeFlow(g.getSource(), 2*i, 1));
        g.pushEdge(EdgeFlow(2*i + 1, g.getTarget(), 1));
        for (int j = 0; j < n; ++j) {
            cin >> a[i][j];
            if (a[i][j]) {
                g.pushEdge(EdgeFlow(2*i, 2*j + 1, 1));
            }
        }
    }
    ...
    vector <vertex> ans = getMaxIndependent(g, n);
    cout << ans.size() << "\n";
    for (int i : ans) {
        cout << i + 1 << " ";
    }
