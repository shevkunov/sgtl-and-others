...
int gVin(int i, int j) {
    return 2*(m * i + j);
}

int gVout(int i, int j) {
    return 2*(m * i + j) + 1;
}

const flow_t INF = 1E6;
int main() {
    cin >> n >> m;
    cin >> d >> w;
    vector <vector<int>> flush(n, vector<int>(m, INF));
    for (int i = 0; i < d; ++i) {
        int x, y;
        cin >> x >> y;
        --x; --y;
    ...
    Network g(2*n*m + 2, 2*n*m, 2*n*m + 1);
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            g.pushEdge(EdgeFlow(gVin(i, j), gVout(i,j), flush[i][j]));

            for (int Si = -1; Si <= 1; ++Si) {
                for (int Sj = -1; Sj <= 1; ++Sj) {
                    if (abs(Si) + abs(Sj) == 1) {
                        if (inBound(i + Si, j + Sj)) {
                            g.pushEdge(EdgeFlow(gVout(i, j), gVin(i + Si, j + Sj), INF));
                        }
                    }
                }
            }
    ...
    cin >> xA >> yA >> xB >> yB;
    --xA; --xB; --yA; --yB;
    g.pushEdge(EdgeFlow(g.getSource(), gVin(xA, yA), INF));
    g.pushEdge(EdgeFlow(gVout(xB, yB), g.getTarget(), INF));

    flow_t flow = Dinitz::getMaxFlow(g);
    if (flow >= INF) {
        cout << -1;
        return 0;
    }
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            if (order[gVin(i,j)] && !order[gVout(i, j)] && (flush[i][j] == 1)) {
