...
int main() {
    ...
    while (q--) {
        ...
        vector<int> girls(k, INF);
        for (size_t i = 0; i < boys.size(); ++i) {
            for (size_t j = 0; j < favor[boys[i]].size(); ++j) {
                girls[favor[boys[i]][j]] = std::min(girls[favor[boys[i]][j]], boys[i]);
            }
        }


        Network<EdgeFlow> g(n + k + 2, n + k, n + k + 1);
        int sexyGirls = 0;
        for (size_t i = 0; i < girls.size(); ++i) {
            if (girls[i] != INF) {
                sexyGirls += (girls[i] != INF);
                g.pushEdge(EdgeFlow(n + i, g.getTarget(), 1));
            }
        }
        for (size_t i = 0; i < favor.size(); ++i) {
            g.pushEdge(EdgeFlow(g.getSource(), i, 1));
            for (size_t j = 0; j < favor[i].size(); ++j) {
                if ((girls[favor[i][j]] != INF) && (i < girls[favor[i][j]])) {
                    g.pushEdge(EdgeFlow(i, n + favor[i][j], 1));
                }
            }
        }
        if (sexyGirls == Dinitz<Network<EdgeFlow>>::getMaxFlow(g)) {
            cout << "Yes\n";
        } else {
            cout << "No\n";
        }
