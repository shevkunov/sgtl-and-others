...
template <class T>
bool dfsFillPath(sgtl::Network& g, sgtl::vertex v, sgtl::vertex es, sgtl::vertex ef, T lambda) {
    ordered[v] = true;
    for(auto i = g.begin(v); i != g.end(v); ++i) {
        if ((*i).flow() > 0) {
            if ((((*i).from() == es) || (es == ef))&& ((*i).to() == ef)) {
                lambda(i);
                return true;
            }
            if (!ordered[(*i).to()]) {
                if (dfsFillPath(g, (*i).to(), es, ef, lambda)) {
                    lambda(i);
                    return true;
                }
            }
...
int main() {
    ...
    sgtl::flow_t flow;
    std::cout << (flow = sgtl::Dinitz::getMaxFlow(g)) << "\n";

    int t;
    std::cin >> t;
    while (t--) {
        ordered.assign(g.vertexCount(), 0);
        int type, x, y;
        std::cin >> type >> x >> y;
        --x; --y; // Zero-numeration
        ...
        if (type == 1) {
            ...
            if ((*i).unsaturated()) {
                ...
                int tmp = 0;
                auto lambda = [&tmp, x, y](sgtl::Network::base_iterator i) {
                    if (((*i).from() == (size_t)x) && ((*i).to() == (size_t)y)) {
                        tmp = (*i).capacity() == (*i).flow();
                        tmp =  0 - tmp;
                        i.pushFlow(tmp);
                        i.pushCapacity(tmp);
                    } else {
                ...
                dfsFillPath(g, g.getSource(), x, y, lambda);
                dfsFillPath(g, y, g.getTarget(), g.getTarget(), lambda);
                --flow;
            }
        }
        std::cout << (flow += sgtl::Dinitz::getMaxFlow(g)) << "\n";
