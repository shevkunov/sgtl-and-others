...
namespace sgtl{
    ...
    void dijkstra(Graph<E> g, vertex v, std::vector<int> &shortest, std::function<int(const E& e)> cost, Weight INF) {
...
namespace sgtl{
    ...
    void fordBellman(Graph<E> g, vertex v, std::vector<int> &shortest,
                     std::function<int(const E& e)> cost, Weight INF) {
        shortest.assign(g.vertexCount(), INF);
        shortest[v] = Weight();
        bool modified = true;
        while (modified) {
...
template<class E>
void dfsDecompose01Flow(Network<E>& g, std::vector<E>& ans, vertex v) {
    for (auto e = g.begin(v); e != g.end(v); ++e) {
...
template<class E>
std::vector<std::vector<E>> decompose01Flow(Network<E>& g) {
...
int main() {
    int n, m, k;
    cin >> n >> m >> k;
    Network<EdgeCostFlowMarked> g(n + 1, 0, n);
    for (int edgeIndex = 1; edgeIndex <= m; ++edgeIndex) {
        int fr, to, cost;
        cin >> fr >> to >> cost;
        g.pushEdge(EdgeCostFlowMarked(--fr, --to, 1, cost, edgeIndex));
        g.pushEdge(EdgeCostFlowMarked(to, fr, 1, cost, edgeIndex));
    }
    g.pushEdge(EdgeCostFlowMarked(n - 1, g.getTarget(), k, 0, m + 1));
    flow_t flow;
    int cost;
    int INF = 2*1E9 + 1E7;
    minCostMaxFlow(g, flow, cost, INF);

    if (flow != k) {
        cout << - 1;
    } else {
        cout.precision(17);
        cout << (double)cost / (double)k << "\n";
        vector<vector<EdgeCostFlowMarked>> paths = decompose01Flow<EdgeCostFlowMarked>(g);
        for (size_t son = 0; son < paths.size(); ++son) {
            if (!son) {
            cout << paths[son].size() - 1;
            } else {
                cout << paths[son].size();
            }
            for (size_t i = 0; i < paths[son].size(); ++i) {
                if (0 - paths[son][i].mark <= m) {
                    cout << " " << 0 - paths[son][i].mark;
                }
            }
            cout << "\n";
        }
