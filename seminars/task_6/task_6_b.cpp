...
int main() {
    ...
    std::pair<int, int> ans = std::make_pair((int)s.length(), 1); // minLen; loopSize - 1
    auto z = sgtl::zFunction(s);
    for (size_t i = 1; i + 2 < z.size(); ++i) {
        int loopSize = i;
        int readyAns = z.size() - z[i];
        ans = std::min(ans, std::make_pair(readyAns, loopSize));
    }

    std::string sans = s.substr(s.length() - ans.first, s.length());
    sans.insert(ans.second, "(");
