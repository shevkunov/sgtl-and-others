    ...
    public:
        struct Vertex {
            ...
            Vertex(size_t size, int parent, int parentChar)
               : leaf(false), linkLeafed(false), link(-1),
        ...
        int getLink(int vertex) {
            ...
            t[vertex].linkLeafed = t[t[vertex].link].linkLeafed || t[vertex].leaf; //*
...
bool searchCycle(sgtl::AhoCorasik &aho, int vertex, std::vector<int> &colors) {
    switch (colors[vertex]) {
    case 0:
        if (aho.t[vertex].linkLeafed) {
            return false;
        }
        colors[vertex] = 1;
        break;
    ...
    for (int e = 0; e < 2; ++e) {
        if (searchCycle(aho, aho.go(vertex, e), colors)) {
            return true;
        }
    }
...
int main() {
    ...
    std::vector<int> colors(aho.t.size(), 0);
    if (!searchCycle(aho, 0, colors)) {
        std::cout << "NIE\n";
    } else {
        std::cout << "TAK\n";
    }
