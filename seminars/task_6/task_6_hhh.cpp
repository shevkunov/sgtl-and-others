...
int main() {
    ...
    std::vector<std::vector<char>> anst(sl, std::vector<char>(sl, 0));

    for (size_t i = 0; i < sl; ++i) {
        sgtl::zFunctionPure(s, z, i);

        int mn = 1;
        for (size_t j = s.length() - 1; j >= sl + i + 1; --j) {
            if (z[j] >= mn) {
                for (size_t l = mn; (l <= z[j]) && (sl + i + 1 + l <= j); ++l) {
                    anst[j - sl - 2][j - sl - 2 + l - 1] = true;
                    mn = l + 1;
                }
            }
        }
